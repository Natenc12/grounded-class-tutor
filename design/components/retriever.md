...
| **Embedding-provider error / timeout — TRANSIENT** (`TransientEmbeddingError`) | propagate as a transport error → `ask()` converts it to Grounder **ERROR**, kind `provider_transient` (never a refusal; ADR 0016). Retrieval itself does not retry — the ask-level retry budget lives in the Grounder, and V1 grants the query embed none of its own. |
| **Embedding-provider error — TERMINAL** (bad key, malformed request) | **raises, and is meant to.** `providers/base.py` wraps only the retryable class, leaving terminal errors as the provider's own types precisely so nothing above has to know them; enumerating them at the `ask()` seam would re-import the coupling ADR 0013's interface exists to prevent. A misconfigured key is a caller/config bug and must look like one, not like a tidy provider ERROR over a healthy corpus. Slice 3's API adapter owns turning it into a 5xx. |
