...
| `attempts` | int | retry count vs the ADR 0028 budget (transient only; terminal skips retries, ADR 0020). Counts CLAIMS, so a crash costs one exactly as a caught 429 does |
