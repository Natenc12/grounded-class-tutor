...
multi-megabyte one nonetheless comes back 413. It is refused before this module runs, by a bound on
the request BODY that every route shares (`gct.api.limits`, issue #125) - a different quantity with
a different owner. `create_class` still accepts any name it is handed, so a caller that is not an
HTTP request is unaffected, which is what keeps the rule above one-writer rather than two.
